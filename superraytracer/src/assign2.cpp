...
bool Assignment2::init()
{
	...
	m_scene.setLightPos(gml::vec4_t(3.0f, 1.0f, 3.0, 1.0));
...
void Assignment2::idle()
{
	...
	if ( currTime - m_lastIdleTime > 0 )
	{
		// Animate the light
		const float angularSpeed = 0.5f * m_rotationSpeed; // 45 degrees / sec
		float angle = angularSpeed * (currTime - m_lastIdleTime);

		gml::mat4x4_t rotMat = gml::rotateYh(angle);
		m_scene.setLightPos(gml::mul(rotMat, m_scene.getLightPos()));
	}
